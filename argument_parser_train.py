...
def parse_args():
    ...
    parser.add_argument(
        '--data_path', 
        help='directory that contains cub files', 
        required=True
    )
    # 训练图所在的子目录【本次=images】
    parser.add_argument(
        '--image_sub_path_train', 
        default='images', 
    ...
    parser.add_argument(
        '--n_pro', 
        help='number of prototypes to generate prompts', 
        default="17,14,11,8,5", 
        type=str
    ) # TODO
    ...
    parser.add_argument(
        '--presence_loss_type', 
        default="original", 
        choices=["original", "soft_constraint", "tanh", "soft_tanh"], 
        type=str
    )
    ...
    parser.add_argument(
        '--enforced_presence_loss', 
        default=2.0, 
        type=float
    )
    # enforced presence 损失的具体形式【本次=enforced_presence】
    parser.add_argument(
        '--enforced_presence_loss_type', 
        default="enforced_presence", 
        choices=["linear", "log", "mse", "enforced_presence"], 
        type=str
    )
    # 像素级熵损失权重【本次=1】(让每个像素尽量只归属一个部件)
    parser.add_argument(
        '--pixel_wise_entropy_loss', 
        default=1.0, 
        type=float
    )
    # 下面这一组 imbalanced_noised_topk 是另一种分类损失分支(本次未开，相关超参不生效)
    parser.add_argument(
        '--use_imbalanced_noised_topk', 
        default=False, 
        action='store_true'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--classifier_type', 
        default="linear", 
        choices=["linear", "independent_mlp"], 
        type=str
    )
    ...
    parser.add_argument(
        ...
        default=False, 
        action='store_true', 
        help='Whether to run as an array job'
             '(i.e. training with multiple random seeds on the same settings)'
    )
